...
export default function GlobeInner({ width, height }: { width: number; height: number }) {
  ...
  useEffect(() => {
    // Dual strategy: onGlobeReady + polling interval as fallback
    // This guarantees autoRotate is set even if onGlobeReady fires before ref is set
    let attempts = 0;
    ...
    const interval = setInterval(() => {
      ...
      try {
        ...
        // controls not ready yet, keep polling
      }
        ...
        onGlobeReady={() => {
          if (globeEl.current) {
            try {
              ...
              // polling interval will handle this
            }
          }
        }}
